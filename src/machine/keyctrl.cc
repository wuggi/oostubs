...
void Keyboard_Controller::set_repeat_rate (unsigned char speed, unsigned char delay) {
    ...
    do{
        status = ctrl_port.inb();
    } while((status & inpb) != 0);          //wait for inputbuffer to be empty

    data_port.outb(cmd_set_speed);          //write speed to data-port

    do{
        status = ctrl_port.inb();
    }while((status & outb) == 0);           //wait for set_speed to be written

    do{
        status = data_port.inb();
    }while((status & ack) == 0);            //wait for ACK-Byte

    data_port.outb(outb_rr);                //this is it!
  
}



void Keyboard_Controller::set_led (Leds led, bool on) {	int status;

    do {
        status = ctrl_port.inb();
    } while((status & inpb) != 0);          //wait for inputbuffer to be empty

    data_port.outb(cmd_set_led);            //write led to data-port

    do{
        status = ctrl_port.inb();
    } while((status & inpb) != 0);          //wait for set_led to be written

    do{
        status = ctrl_port.inb();
    } while((status & outb) == 0);          //wait for response

    status = data_port.inb();
    if ((status & ack) == 0){               //wait for ACK-Byte

        if(on){
            leds |= led;                    //leds = leds | led
        } else {
            leds &= ~led;                   //leds = leds & ¬led
        }
